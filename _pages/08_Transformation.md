...
**Ana(morphism)** `T` &rarr; `IObservable<T>`

**Cata(morphism)** `IObservable<T>` &rarr; `T`

**Bind** `IObservable<T1>` &rarr; `IObservable<T2>`
...
**A**na enters the sequence. `T` &rarr; `IObservable<T>`

**B**ind modifies the sequence. `IObservable<T1>` &rarr; `IObservable<T2>`

**C**ata leaves the sequence. `IObservable<T>` &rarr;s `T`
...
In LINQ and therefore Rx, the `bind` method is `SelectMany`.
...
One of my colleagues helped me understand `SelectMany` better by suggesting I think of it as “from one, select many”. 
An even better definition is “From one, select zero or more”. 
...
While this method does reduce a sequence, it is not a fit for a functional *fold* as the result is still a sequence. 
Taking this into account, we find that `Where` is actually a fit for *bind*. 
...
If we process that with a `SelectMany` operator that returns a sequence of [x\*10, (x\*10)+1, (x\*10)+2], we would get the [[10,11,12], [20,21,22],
[30,31,32]].
...
The resulting child sequences are flattened into
...
This is the marble diagram representation of the sequence [1,2,3] spaced three seconds apart (note each character represents one second).
...
As we saw in the earlier example, the simple implementation of just using select is as follows:
...
What we have learnt up to this point however can all largely be applied to `IEnumerable<T>` sequences too. 
Rx can be much more complex than what many people will have dealt with in `IEnumerable<T>` world, as we have seen with the `SelectMany` operator. 
