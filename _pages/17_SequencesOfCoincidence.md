---
layout: page
permalink: /17_SequencesOfCoincidence.html
...
We can conceptualize events that have *duration* as *windows*. 
For example;
...
As
the `Window` operators ensure that the windows (child sequences) do not overlap, we can use either of the `Concat`, `Switch` or `Merge` operators to turn a windowed sequence back into its original form.
...
If we now want to extend this method to offer skip functionality, we need to have two different sequences: one for opening and one for closing. 
We open a window on subscription and again after the `skip` items have passed. 
We close those windows after `count` items have passed since the window opened.
...
For an alternative summary of reducing operators to a primitive set see Bart DeSmet's [excellent MINLINQ post](http://blogs.bartdesmet.net/blogs/bart/archive/2010/01/01/the-essence-of-linq-minlinq.aspx "The essence of LINQ - MinLINQ")
