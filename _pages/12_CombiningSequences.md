---
layout: page
permalink: /12_CombiningSequences.html
...
Below is a marble diagram of the `Concat` operator applied to the `GetSequences` method. 's1', 's2' and 's3' represent sequence 1, 2 and 3. 
...
It is not the same as a `BehaviorSubject<T>` however, as it will not cache the last value.
...
It is a non-deterministic function, first introduced by John McCarthy and is an abbreviation of the word *Ambiguous*. 
...
`Merge` also has the overload that takes an `IObservable<IObservable<T>>`, a nested observable. 
To summarize:
...
We then receive the number '2' from `N`. 
The last letter is still 'a' so the next pair is '2a'. 
