---
layout: page
permalink: /13_TimeShiftedSequences.html
...
When consuming an `IEnumerable<T>` sequence, asynchrony is not a concern; when we call `MoveNext()`, we are blocked until the sequence yields. 
...
The `skip` parameter cannot be less than or equal to zero. 
If you want to use a value of zero (i.e. each buffer contains all values), then consider using the [*Scan*](07_Aggregation.html#Scan) method instead with an `IList<T>` as the accumulator.

Let's see each of these in action. 
In this example, we have a source that produces values every second. 
