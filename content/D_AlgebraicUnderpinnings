...
public class OverSimplifiedWhereObserver<T> : IObserver<T>
{
    ...
    public OverSimplifiedWhereObserver(IObserver<T> downstreamSubscriber, Func<T, bool> predicate)
    {
...
Critically, to qualify as a monad, the two operations just described (return and bind) must conform to certain rules, or _laws_ as they are often described in the literature. There are three laws, and all of them govern how the bind operation works, and two of these are concerned with how return and bind interact with one another. These laws are the foundation of the composability of operations based on monads. The laws are somewhat abstract, so it isn't exactly obvious _why_ they enable this, but they are non-negotiable. If your type and operations don't follow these laws, then you don't have a monad, so you can't rely on the characteristics monads guarantee.
...
Suppose we have two functions, `Tx1` and `Tx2`, each of form suitable for passing as the argument to `SelectMany`. There are two ways we could apply these:
...
*  _id_ ∘ _f_ is equivalent to just _f_
*  _f_ ∘ _id_ is equivalent to just _f_
* (_f_ ∘ _g_) ∘ _s_ is equivalent to _f_ ∘ (_g_ ∘ _s_)
...
Although in general a catamorphism can strip off the container (e.g., `Sum` for `IEnumerable<int>` produces an `int`), this isn't absolutely necessary, and with Rx most catamorphisms don't do this. As described in the threading and scheduling chapter's [Lock-ups](11_SchedulingAndThreading.md#lock-ups) section, blocking some thread while waiting for a result that will only occur once an `IObservable<T>` has done something in particualr (e.g., if you want to calculate the sum of items, you have to wait until you've seen all the items) is a recipe for deadlock in practice.
